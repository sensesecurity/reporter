    ...
    if response.status_code == 200:
        ...
        def sort_by_label(issue):
            ...
            for label in labels:
                # Get the label name
                name = label["name"]
                # If the name is in the label values dictionary, compare the value with the highest label value
        ...
        issue_numbers = {"High": 0, "Medium": 0, "Low": 0}

        # Loop through the sorted issues
        for issue in issues:
            # Get the label name of the issue
            label_name = issue["labels"][0]["name"]
            # If the label name is in the label values dictionary, increment the issue number for that label
            if label_name in label_values:
                issue_numbers[label_name] += 1
                # Generate the prefix with the letter and the number
                prefix = f"[{label_name[0]}-{issue_numbers[label_name]:02d}]"
